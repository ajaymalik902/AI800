...
def mark_img(img_path: str, dest_dir=None):
    ...
    mask_path = img_path.split('\\')[2].replace(".jpg", "_masks\\")
    ...
    if dest_dir:
        ...
        save_path = "raw_images/" + mask_path
...
def create_marked_images():
    ...
    for images in x[:850]:
        t = threading.Thread(target=worker, args=(images, "train_data"))
        t.start()
    for images in x[850:]:
        t = threading.Thread(target=worker, args=(images, "test_data"))
        t.start()
    print("done")
